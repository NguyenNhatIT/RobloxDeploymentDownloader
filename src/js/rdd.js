/*
    rdd - https://github.com/NguyenNhatIT/RobloxDeploymentDownloader
    Copyright (C) 2024-2025 Nguyễn Minh Nhật <github.com/NguyenNhatIT> | MIT License
*/
...
// Use the consoleText from the global scope instead of redeclaring it
function getConsoleText() {
    return window.consoleText || document.getElementById("consoleText");
}

function getDownloadForm() {
    return document.getElementById("downloadForm");
}

function rddLog(msg = "", end = "\n", autoScroll = true) {
    const consoleText = getConsoleText();
    // Use the UI console system if available, otherwise fallback to direct DOM manipulation
    if (window.addConsoleMessage) {
        window.addConsoleMessage(msg + end, 'log');
    } else {
        // Fallback for when UI console isn't ready yet
        const messageElement = document.createElement('div');
        messageElement.className = 'console-line console-log';
        messageElement.textContent = msg + end;
        consoleText.appendChild(messageElement);
...
function rddGetPermLink() {
    const downloadForm = getDownloadForm();
    if (!downloadForm) return basePath;
...
function rddDownloadFromForm() {
    window.open(rddGetPermLink(), "_self");
};

function rddCopyPermLink() {
    navigator.clipboard.writeText(rddGetPermLink());
};

function scrollToBottom() {
    const consoleText = getConsoleText();
    if (consoleText && consoleText.parentElement) {
...
function requestBinary(url, callback) {
    ...
    httpRequest.onload = function() {
        ...
        if (statusCode != 200) {
            rddLog(`[!] Binary request error (${statusCode}) @ ${url}`);
            return;
        }

        const arrayBuffer = httpRequest.response;
        if (! arrayBuffer) {
            rddLog(`[!] Binary request error (${statusCode}) @ ${url} - Failed to get binary ArrayBuffer from response`);
            return;
        }
    ...
    httpRequest.onerror = function(e) {
        rddLog(`[!] Binary request error @ ${url} - ${e}`);
    };
...
function main() {
    if (window.location.search === "") {
        // Use the UI function to show the form
        if (window.showDownloadForm) {
            window.showDownloadForm();
        }
        rddLog(usageMsg, "\n", false);
        return;
    }
    ...
    if (version && ! binaryType) {
        rddLog("[!] Error: If you provide a specific `version`, you need to set the `binaryType` aswell! See the usage doc below for examples of various `binaryType` inputs:", "\n\n");
        rddLog(usageMsg, "\n", false);
        return;
    }
    ...
    if (compressZip !== null) {
        if (compressZip !== "true" && compressZip !== "false") {
            rddLog(`[!] Error: The \`compressZip\` query must be "true" or "false", got "${compressZip}"`);
        }

        compressZip = (compressZip === "true");
    } else {
        const downloadForm = getDownloadForm();
        compressZip = downloadForm ? downloadForm.compressZip.checked : false;
    }
    ...
    if (compressionLevel !== null) {
        try {
            ...
            rddLog(`[!] Error: Failed to parse \`compressionLevel\` query: ${err}`, "\n\n");
            rddLog(usageMsg, "\n", false);
            return;
        }

        if (compressionLevel > 9 || compressionLevel < 1) {
            rddLog(`[!] Error: The \`compressionLevel\` query must be a value between 1 and 9, got ${compressionLevel}`, "\n\n");
            rddLog(usageMsg, "\n", false);
            return;
        }
    } else {
        const downloadForm = getDownloadForm();
        compressionLevel = downloadForm ? downloadForm.compressionLevel.value : 5; // Only applies to when `compressZip` is true aswell
    }

    // At this point, we expect `binaryType` to be defined
    if (! binaryType) {
        rddLog("[!] Error: Missing required \`binaryType\` query, are you using an old perm link for a specific version?", "\n\n");
        rddLog(usageMsg, "\n", false);
        return;
    }
    ...
    if (binaryType in binaryTypes) {
        ...
        rddLog(`[!] Error: \`binaryType\` "${binaryType}" not supported. See below for supported \`binaryType\` inputs:`, "\n\n");
        rddLog(usageMsg);
        return;
    }
    ...
    if (version) {
        ...
        rddLog("Copy the version hash (the area with \"version-xxxxxxxxxxxxxxxx\" in double-quotes) from the page in the link below (we can't because of CORS), and paste it in the field named \"Version Hash\" in the form above\n");
        
        // Use the UI function to add the link
        if (window.addClientSettingsLink) {
            window.addClientSettingsLink(clientSettingsUrl);
        } else {
            // Fallback
            const consoleText = getConsoleText();
        ...
        // Update form values using the UI function
        if (window.updateFormValues) {
            window.updateFormValues(channel, binaryType, compressZip, compressionLevel);
        } else {
            const downloadForm = getDownloadForm();
            if (downloadForm) {
                // Fallback
        ...
        // Show the form using the UI function
        if (window.showDownloadForm) {
            window.showDownloadForm();
        }
...
async function fetchManifest() {
    ...
    if (binaryType === "MacPlayer" || binaryType === "MacStudio") {
        ...
        rddLog(`[+] Fetching zip archive for BinaryType "${binaryType}" (${zipFileName})`);

        const outputFileName = `${channel}-${binaryType}-${version}.zip`;
        rddLog(`[+] (Please wait!) Downloading ${outputFileName}..`, "");
        ...
        rddLog(`[+] Fetching rbxPkgManifest for ${version}@${channel}..`);
        ...
        var manifestBody = "";
        {
            ...
            if (! resp.ok) {
                rddLog(`[!] Failed to fetch rbxPkgManifest: (status: ${resp.status}, err: ${(await resp.text()) || "<failed to get response from server>"})`);
                return;
            }
...
async function downloadZipsFromManifest(manifestBody) {
    ...
    if (pkgManifestLines[0] !== "v0") {
        rddLog(`[!] Error: unknown rbxPkgManifest format version; expected "v0", got "${pkgManifestLines[0]}"`);
        return;
    }
    ...
    if (pkgManifestLines.includes("RobloxApp.zip")) {
        ...
        if (binaryType === "WindowsStudio64") {
            rddLog(`[!] Error: BinaryType \`${binaryType}\` given, but "RobloxApp.zip" was found in the manifest!`);
            return;
        }
        ...
        if (binaryType === "WindowsPlayer") {
            rddLog(`[!] Error: BinaryType \`${binaryType}\` given, but "RobloxStudio.zip" was found in the manifest!`);
            return;
        }
    } else {
        rddLog("[!] Error: Bad/unrecognized rbxPkgManifest, aborting");
        return;
    }

    rddLog(`[+] Fetching blobs for BinaryType \`${binaryType}\`..`);
...
`);
    ...
    function checkIfNoThreadsLeft() {
        ...
        rddLog();
        if (compressZip) {
            rddLog(`[!] NOTE: Compressing final zip (with a compression level of ${compressionLevel}/9), this may take a minute`);
        }

        rddLog(`[+] Exporting assembled zip file "${outputFileName}".. `, "");
...
async function downloadPackage(packageName, doneCallback, getThreadsLeft) {
    ...
    requestBinary(blobUrl, async function(blobData) {
        if (packageName in binExtractRoots == false) {
            rddLog(`[*] Package name "${packageName}" not defined in extraction roots for BinaryType \`${binaryType}\`, skipping extraction! (THIS MAY MAKE THE ZIP OUTPUT INCOMPLETE, BE AWARE!)`);
            zip.file(packageName, blobData);
            rddLog(`[+] Moved package "${packageName}" directly to the root folder`);
            doneCallback();
            return;
        }

        rddLog(`[+] Extracting "${packageName}"..`);
        ...
        rddLog(`[+] Extracted "${packageName}"! (Packages left: ${getThreadsLeft()})`);
        doneCallback();
    });
};

// Make RDD functions available globally but with unique names to avoid conflicts
window.rddDownloadFromForm = rddDownloadFromForm;
window.rddCopyPermLink = rddCopyPermLink;
window.rddLog = rddLog;